...
if __name__ == '__main__':
    # data/projections.csv is now produced by the v2 pipeline (v2/to_csv.py).
    # Running this file would silently replace it with v1 output and quietly
    # revert the optimiser, planner, simulator and web app a generation.
    import sys
    if '--force-v1' not in sys.argv:
        print(__doc__.strip().splitlines()[0])
        print('\nSUPERSEDED. data/projections.csv is generated by the v2 pipeline:')
        print('    .venv/bin/python v2/player_model.py && .venv/bin/python v2/to_csv.py')
        print('\nRunning this would overwrite it with v1 projections. If that is')
        print('really what you want, pass --force-v1. v1 output is archived at')
        print('data/projections_v1.csv.')
        raise SystemExit(1)
