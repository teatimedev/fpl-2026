...
test('price rise invalidates transfer budget; injury invalidates projections', () => {
  assert.equal(check(data, { ...live, elements: new Map([...live.elements, [1, { now_cost: 51, status: 'a', news: '' }]]) }).digestReady, false)
  assert.equal(check(data, { ...live, elements: new Map([...live.elements, [1, { now_cost: 50, status: 'i', news: 'Injured' }]]) }).projectionsReady, false)
})
...
test('a future build date, broken deadline or missing player cannot pass coherence', () => {
  assert.equal(check({ ...data, meta: { ...data.meta, generated: '2026-09-07T00:00:00Z' } }).projectionsReady, false)
  assert.equal(check(data, { ...live, deadline: 'broken' }).projectionsReady, false)
  assert.equal(check(data, { ...live, elements: new Map([...live.elements].slice(1)) }).projectionsReady, false)
  assert.equal(check(data, live, NaN).digestReady, false)
})
test('availability changes invalidate projections even if status and news are unchanged', () => {
  const elements = new Map(live.elements)
  elements.set(1, { ...elements.get(1), chance_of_playing_next_round: 25 })
  assert.equal(check(data, { ...live, elements }).projectionsReady, false)
})
