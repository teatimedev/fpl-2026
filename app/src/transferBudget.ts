...
import { adviceStatus } from './coherence.ts'
...
export function accountSellingValues(D: Data, live: LiveState | null, ids: number[],
  ...
  // Sale values are only valid for the squad and account the plan reconciled.
  if (!live || !adviceStatus(D, live, ids, bank, ft, entryId, now).planUsable) return null
