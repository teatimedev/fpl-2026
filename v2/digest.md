# FPL weekly — Gameweek 4

Deadline **Sat 12 Sep, 12:30 UTC** — 6d 1h away.
Projections cover GW4–9.

## Captain: **Haaland** (MCI)

Projected 6.8 this week, doubled to 13.7. Vice: Saka (5.8).

| rank | player | club | this GW | start % |
|---|---|---|---|---|
| 1 | Haaland | MCI | 6.8 | 95 |
| 2 | Saka | ARS | 5.8 | 91 |
| 3 | Mbeumo | MUN | 5.2 | 94 |
| 4 | Raya | ARS | 4.9 | 97 |

## Starting XI

| pos | player | club | this GW | GW4–9 |
|---|---|---|---|---|
| GKP | Raya | ARS | 4.9 | 27.6 |
| DEF | Calafiori | ARS | 4.9 | 28.3 |
| DEF | Tarkowski | EVE | 4.1 | 24.8 |
| DEF | Thiaw | NEW | 4.0 | 27.2 |
| DEF | Milenković | NFO | 3.3 | 22.6 |
| MID | Saka (V) | ARS | 5.8 | 36.3 |
| MID | Mbeumo | MUN | 5.2 | 34.8 |
| MID | Kluivert | BOU | 4.8 | 27.9 |
| MID | Foden | MCI | 3.9 | 24.8 |
| FWD | Haaland (C) | MCI | 6.8 | 43.5 |
| FWD | Thiago | BRE | 4.8 | 29.8 |

**Bench order:** Leno (2.9), O'Shea (3.1), Sadiki (2.7), Obi (0.6)

## Your lineup vs the model  (FPL entry 3415101, picks from GW3)

- **Vice:** Foden (3.9) is not one of your top three; the model would use Saka.

## Check before the deadline

- **Foden** (MCI, XI) — starts only 64% of the time

## Transfers  (£0.0m in the bank, 3 free)

Gain is the lift to your expected starting XI and captain plus modelled auto-sub cover over GW4–9. The auto-sub term uses each starter's non-appearance risk, not a flat bench weight. A hit costs 4.

| out | in | £ | XI + captain | auto-sub | total | net |
|---|---|---|---|---|---|---|
| Sadiki (SUN) | Gomez (BHA) | +0.0 | +1.8 | +2.6 | **+4.4** | +4.4 |
| Milenković (NFO) | Botman (NEW) | -0.5 | +1.4 | +1.2 | **+2.6** | +2.6 |
| Tarkowski (EVE) | Gvardiol (MCI) | -0.4 | +1.3 | +1.1 | **+2.4** | +2.4 |
| O'Shea (IPS) | Giles (HUL) | +0.0 | +0.8 | +0.7 | **+1.5** | +1.5 |
| Kluivert (BOU) | Barnes (NEW) | +0.0 | -0.4 | +0.8 | **+0.4** | +0.4 |
| Thiago (BRE) | João Pedro (CHE) | -0.3 | -0.7 | +0.9 | **+0.2** | +0.2 |
| Obi (MUN) | Danns (LIV) | +0.0 | +0.0 | +0.1 | **+0.1** | +0.1 |

**Recommended: hold.** Milenković → Ajayi, Sadiki → Gakpo, Foden → Barnes is the move queued — worth taking over the window, but taking it now rather than next week is worth only +0.4 (the plan scores 402.4 against 402.0 with this week held). You would have 4 free transfers next week.

**Best two-move combinations** (net of any hit):

| out | in | £ | XI + captain | auto-sub | total | net |
|---|---|---|---|---|---|---|
| Tarkowski + Sadiki | Botman + Barnes | +0.0 | +5.2 | +3.0 | +8.2 | **+8.2** |
| Tarkowski + Sadiki | Botman + Tavernier | +0.0 | +4.9 | +2.8 | +7.7 | **+7.7** |
| Milenković + Sadiki | Robinson + Barnes | +0.0 | +5.7 | +1.9 | +7.7 | **+7.7** |
| Milenković + Sadiki | Ajayi + Barnes | -0.4 | +5.6 | +1.9 | +7.6 | **+7.6** |
| Milenković + Sadiki | Maatsen + Barnes | +0.0 | +4.7 | +2.8 | +7.5 | **+7.5** |


## The next six weeks, planned


**This Friday under uncertainty:** the planner's proposed GW4 squad beats holding in 62% of 4,000 simulations (average +2.5 points). There is probably a points edge, but its +2.5 average does not clear the 6.0-point bar for spending 3 transfers now.
Best path from here: **402.4** pts (0 hits). Making no move this week and re-planning: 402.0. Acting now is worth **+0.4** across 3 moves — not enough; hold.

- **GW4** 63.7 pts, C Haaland, 3 FT  Milenković → Ajayi, Sadiki → Gakpo, Foden → Barnes
- **GW5** 65.8 pts, C Haaland, 1 FT  hold
- **GW6** 67.3 pts, C Mbeumo, 2 FT  hold
- **GW7** 69.0 pts, C Haaland, 3 FT  Raya → Tzolakis, Tarkowski → Andersen, Gakpo → Palmer
- **GW8** 67.9 pts, C Haaland, 1 FT  Thiago → João Pedro
- **GW9** 68.7 pts, C Haaland, 1 FT  Andersen → Ballard

_The multiweek planner uses a linear bench proxy and small fixture swings can cause churn; treat future moves as directional rather than scripted._

## Price watch

Net transfers this gameweek as a share of current owners. Top of the left column rises soonest, top of the right falls. (Uncalibrated until a few weeks of price logs exist.)

| rising | pressure | net | falling | pressure | net |
|---|---|---|---|---|---|
| Ansah £5.0 | +77900.0% | +779 | Elliott £5.4 | -60600.0% | -606 |
| Mbaye £6.0 | +77200.0% | +772 | Diarra £5.4 | -34400.0% | -344 |
| Vaz £5.0 | +49100.0% | +491 | Humphreys £4.5 | -31900.0% | -319 |
| Palacios £5.0 | +46500.0% | +465 | Morato £5.0 | -30500.0% | -305 |
| Flemming £5.5 | +39600.0% | +396 | Esse £5.0 | -29300.0% | -293 |
| Fofana £5.5 | +34300.0% | +343 | M.Sarr £4.5 | -25600.0% | -256 |

⭑ = in your squad.

**Price risk if you wait (shadow-only):**

- Target Ajayi (net +397k this event) is at rise risk; waiting past the deadline could cost +0.1m [net +397k this event (in 432k / out 35k), sel 7.7% = Q2 (~113k to move), price 4.1 over 5 snaps (+0.1m in window), already +1x0.1m this event]
- Squad player Mbeumo (net -416k this event) at fall risk (-0.1m hurts the bank) [net -416k this event (in 59k / out 476k), sel 30.2% = Q3 (~150k to move), price 8.0 over 5 snaps (+0.0m in window)]
- Squad player Thiago (net -168k this event) at fall risk (-0.1m hurts the bank) [net -168k this event (in 39k / out 207k), sel 14.5% = Q3 (~150k to move), price 8.0 over 5 snaps (+0.0m in window)]

_Based on 5 price snapshots. This warning is not part of the transfer verdict until at least six weeks of actual rises and falls can calibrate it._

---

_Generated 2026-09-06 10:52 UTC. Projections are estimates: hold-out rank correlation is about 0.46, so treat the ordering as a strong hint and the point totals as rough._